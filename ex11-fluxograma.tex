...
\begin{tikzpicture}
    ...
    \path [line] (decisaoMenu.east) -- ++(6,0.01) |- (fimJogo) node[near start, right] {2};
    \path [line] (opcaoInvalida.east) -- ++(1,0) -- (menu.west);
    \path [line] (decisaoMenu.south) -- (texto1) node[midway, right] {1};
    \path [line] (decisaoObj.east) -- ++(1.33,0) |- (morte) node[near start, right] {maior ou = 2};
    \path [line] (texto1) -- (decisaoObj);
    \path [line] (decisaoObj.south) -- (texto2) node[midway, right] {1};
    \path [line] (decisaoFinal.east) -- ++(1.5,0) |- (morte) node[near start, right] {1 ou maior que 2};
    \path [line] (texto2) -- (decisaoFinal);
    \path [line] (decisaoFinal.south) -- (vitoria) node[midway, right] {2};
